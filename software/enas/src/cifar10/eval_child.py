...
class GeneralChild(Model):
  ...
  def _enas_layer(self, layer_id, prev_layers, start_idx, out_filters, is_training):
    ...
    if self.whole_channels:
      ...
      count = self.sample_arc3[start_idx]
      branches = {}
      ...
      count = self.sample_arc3[start_idx:start_idx + 2 * self.num_branches]
      branches = []
      with tf.variable_scope("branch_0"):
    ...
    if layer_id > 0:
      ...
      skip = self.sample_arc3[skip_start: skip_start + layer_id]
  ...
  def reset_idx(self, sess):
    # zc
    # self.idx_arc = 0 * self.idx_arc
    sess.run(self.reset_op)
    print('Reset arc idx to ', sess.run(self.idx_arc))

  def customized_eval_once(self, sess, eval_set, feed_dict=None, verbose=False):
    # zc
    print('Current idx is ', sess.run(self.idx_arc))
    acc = self.eval_once(sess, eval_set, feed_dict, verbose)
    sess.run(self.inc_op)
    return acc
  ...
  def connect_controller(self, controller_model):
    if self.fixed_arc is None:
      ...
      print("Customized arch")
      fixed_arc = np.array([int(x) for x in child_arc.split(" ") if x])
      self.sample_arc = fixed_arc
    # replace all sample_arc -> sample_arc2 when evaluate
    self.sample_arc2 = controller_model.sample_arc2
    self.sample_arc3 = controller_model.sample_arc3
    self.structures = controller_model.structures
    self.idx_arc = controller_model.idx_arc
    self.reset_op = controller_model.reset_op
    self.inc_op = controller_model.inc_op

    self._build_train()
    self._build_valid()
    self._build_test()

  def inc_arc(self, idx):
    print("Reset the arch")
